...
// 혼자 행 높이를 떠받치고 있었다. 이제 th·td가 같은 py를 갖는다 — 셀 콘텐츠
// 박스는 전과 후가 똑같이 37px이고(옛 35px 라인박스 + UA 기본 패딩 1px×2,
// 새 21px 라인박스 + py-xs 8px×2), 여기에 border-collapse가 공유하는 테두리
// 1px이 더해져 실측 행 높이는 양쪽 다 38px이다. 바뀐 것은 높이가 아니라 그
// 높이를 무엇이 만드느냐다 — 셀 세로 정렬이 우연에 기대지 않는다.
...
- 표의 행 높이는 38px 그대로다 — th·td가 같은 py를 갖게 되는 것이 변화이고, 높이 자체는 안 변한다.
