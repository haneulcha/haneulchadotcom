...
- **이 계획의 코드 블록은 한 번도 컴파일된 적이 없다.** 의도를 정확히 전달하려고 쓴 초안이지 정본이 아니다. 타입 오류·API 불일치·잘못된 시그니처가 나오면 계획을 그대로 옮기지 말고 고쳐라 — 판정은 `pnpm type-check`와 `pnpm lint`가 하지 이 문서가 하지 않는다. 계획과 어긋나게 고친 부분은 최종 보고에 남긴다.
