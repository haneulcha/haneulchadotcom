...
- 실행자: **Opus** (CLAUDE.md 「모델 분담」). 이 대화의 맥락 없이 이 문서만 읽고 실행한다. 그래서 모든 결정은 본문에 다시 적혀 있고, 판단이 필요한 지점은 **판단 필요** 로 명시했다 — 그 지점에서 스스로 발명하지 말고 사람/Fable에 확인한다.
...
- **이 계획의 코드 블록은 한 번도 컴파일된 적이 없다.** 의도를 정확히 전달하려고 쓴 초안이지 정본이 아니다. 타입 오류·API 불일치·잘못된 시그니처가 나오면 계획을 그대로 옮기지 말고 고쳐라 — 판정은 `pnpm type-check`와 `pnpm lint`가 하지 이 문서가 하지 않는다. 계획과 어긋나게 고친 부분은 최종 보고에 남긴다. **단, 계획이 그 오류에 대한 처방을 명시해 둔 경우(예: Task 10의 `CSSProperties` 보강)는 그 처방을 먼저 따른다** — 고치기 전에 해당 태스크 본문을 검색해 보라.
...
실행자가 스스로 발명하면 안 되는 것들이다. 각 태스크 본문에도 같은 표시가 있다. 기본값이 주어진 항목은 **기본값으로 일단 구현하고 진행하되**, 최종 확정은 사람/Fable 몫이다.
...
| J4  | 셰이더 미학 튜닝: Gerstner 파라미터, 밴드 경계값, 커스틱 임계값, 포말 폭, bloom 유무 | Task 6–8 | 본문에 기본값 제공. **체크리스트 행이 켜지면 튜닝을 멈추고 Task 13 게이트로 미룬다** |
...
[
  ...
  },
  {
    ...
    "links": [
      {
        "label": "저장소",
        "href": "https://github.com/haneulcha/health-defense-webview-poc"
      }
    ]
  },
  {
    ...
    "links": [
      ...
      {
        "label": "저장소",
        "href": "https://github.com/haneulcha/pourover-work"
      }
    ]
  },
  {
...
이미지 확보와 30초 테스트("아무것도 만지지 않고 30초를 그냥 봤을 때 좋은가")의 판정은 사람/Fable 몫 — 실행자는 스크린샷을 찍어 첨부하는 것까지 한다.
