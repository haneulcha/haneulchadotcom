...
- 기본: th의 `leading-[2.5]` 제거 → `text-body-sm` + `py-xs`(8px). 행 높이는 38px로 그대로다 — 옛 35px 라인박스에 UA 기본 패딩 1px씩이 붙어 37px 콘텐츠 박스였고, 새 21px 라인박스 + py-xs 8px씩도 같은 37px이다. 바뀐 것은 높이가 아니라 그 높이를 무엇이 만드느냐다.
