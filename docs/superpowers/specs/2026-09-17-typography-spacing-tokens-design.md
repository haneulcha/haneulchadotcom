...
- 기본: th의 `leading-[2.5]` 제거 → `text-body-sm` + `py-xs`(8px). 셀 콘텐츠 박스는 전후 모두 37px이고(옛 35px 라인박스 + UA 기본 패딩 1px×2, 새 21px 라인박스 + py-xs 8px×2), border-collapse가 공유하는 테두리 1px이 더해져 실측 행 높이는 양쪽 다 38px이다. 바뀌는 것은 높이가 아니라 그 높이를 무엇이 만드느냐다.
